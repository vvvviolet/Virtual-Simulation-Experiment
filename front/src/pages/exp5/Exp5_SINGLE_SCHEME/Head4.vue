    ...
    <a-table :dataSource="dataSource" :columns="columns" bordered>
        <template #bodyCell="{ column, text, record }">
            <template v-if="column.dataIndex != 'number' && column.dataIndex != 'desc'">
                ...
                @change="updateDataSource(record, column.dataIndex)"
                :defaultValue="getDefaultValue(record.key, column.dataIndex)">
                </a-input>
            </template>
        </template>
    </a-table>
...
export default {
    methods: {
        ...
        */
        ...
        updateDataSource(record, dataIndex) {
            const inputValue = record[dataIndex];
            const defaultValue = this.dataSource.find(data => data.key === record.key)[dataIndex];

            // If the input value is not empty, use the input value; otherwise, use the default value from dataSource
            record[dataIndex] = inputValue.trim() !== '' ? inputValue : defaultValue;
        },
        getDefaultValue(key, dataIndex) {
            return this.dataSource.find(data => data.key === key)[dataIndex];
        }
    },
    data() {
        return {
